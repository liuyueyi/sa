...
int sm2(char *buf1, char *buf2, size_t len, const char *pathname, int (*crypt)(void *handle, char *buf1, char *buf2, size_t len, const char *pathname))
{
     int ret, i;
    void *handle, *session;
...
err_session:
	SDF_CloseDevice(handle);
err_device:
	return ret;
}


int do_encrypt(void *handle, char *plain_text, char *result, size_t len, const char *pk_pathname)
{
    int ret = 0;
...
int do_decrypt(void *session, char *cipher, char *result, size_t len, const char *sk_pathname)
{
    int ret = 0;
    unsigned int uiAlgID = SGD_SM2_3;
	...
	ret = SDF_ExternalDecrypt_ECC(session, uiAlgID, pucPrivateKey, pucEncData, recover, &length);
	if(ret)
        fprintf(stderr, "failed to decrypt the cipher (err=%d)\n", ret);
...
int do_hash(void *session, char *buf1, char *buf2, size_t len, const char *pathname)
{
